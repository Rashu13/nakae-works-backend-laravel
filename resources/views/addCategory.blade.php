                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        <div class="p-2" style="background: rgba(255,255,255,0.15);">
                            <i class="mdi mdi-shape fs-5 text-white"></i>
                        </div>
                        <div>
                            <h6 class="text-white fw-bold mb-0" style="font-size: 1.1rem;">Category Management</h6>
                            <span class="text-white-50" style="font-size: 0.75rem;">Manage service categories, icons, and display order</span>
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-white text-dark px-3 py-2 fw-bold" style="font-size: 0.75rem;">
                            {{ $cates->count() }} Total Categories
                        </span>
                        <button type="button" class="btn btn-success text-white fw-bold px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#addCategoryModal">
            ...
            <div class="compact-card p-0">
                <div class="compact-card-header">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h6 class="mb-0 fw-bold text-dark" style="font-size: 0.85rem;">Category Directory</h6>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-light text-secondary border fw-semibold" style="font-size: 0.7rem;">
                            {{ $cates->count() }} Total
                        </span>
                        <button type="button" class="btn btn-sm btn-primary px-3" data-bs-toggle="modal" data-bs-target="#addCategoryModal">
                ...
                <div class="table-responsive">
                    <table class="table-compact-dense align-middle">
                        ...
                        <tbody>
                            @forelse($cates as $key => $cate)
                                <tr>
                                    <td class="text-center text-muted fw-bold">{{ $key + 1 }}</td>

                                    <!-- Category Image -->
                                    <td>
                                        <img src="{{ asset($cate->category_image) }}" class="img-thumb-sm" alt="Img">
                                    </td>

                                    <!-- Category Icon -->
                                    <td class="text-center">
                                        <img src="{{ asset($cate->category_icon) }}" class="icon-thumb-sm" alt="Icon">
                                    </td>

                                    <!-- Category Name -->
                                    <td>
                                        <span class="fw-bold text-dark d-block">{{ $cate->category_name }}</span>
                                        <span class="text-muted small" style="font-size: 0.68rem;">ID: #{{ $cate->id }}</span>
                                    </td>

                                    <!-- Sort Order -->
                                    <td class="text-center">
                                        <span class="badge bg-light text-dark border px-2 py-0 fw-semibold" style="font-size: 0.7rem;">
                                            {{ $cate->sort_order }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-inline-flex gap-1">
                                            ...
                                            <!-- Delete -->
                                            <form action="{{ route('admin.category.delete', $cate->id) }}"
                                                  method="POST"
                                                  class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        ...
                                                        style="width: 26px; height: 26px;"
                                                        onclick="return confirm('Are you sure you want to delete {{ $cate->category_name }}?')"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 shadow-lg">
                        <div class="modal-header text-white p-3" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%) !important;">
                            <h6 class="modal-title fw-bold text-white mb-0" id="addCategoryModalLabel">
                        ...
                        <form action="{{ route('admin.category.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="modal-footer bg-light p-2 px-3 border-top">
                                ...
                                <button type="submit" class="btn btn-primary px-4 py-1 text-uppercase fw-bold">
                                    <i class="mdi mdi-check-circle me-1"></i> Save Category
                                </button>
                            </div>
                        </form>
                    </div>
