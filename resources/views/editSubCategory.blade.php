            ...
            <div class="compact-card p-3">
                <form action="{{ route('admin.subcategory.update', $subCate->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-2 mb-2">
                        ...
                        <!-- Description (Desc) -->
                        <div class="col-12 mt-2">
                            <label class="info-label-sm">Service Description (Inclusions, Exclusions, Details)</label>
                            <textarea name="desc"
                                      rows="6"
                                      class="form-control @error('desc') is-invalid @enderror"
                                      style="min-height: 140px; font-size: 0.85rem; line-height: 1.5; resize: vertical;"
                                      placeholder="Enter detailed service description, key features, inclusions, warranties, or specific instructions for customers...">{{ old('desc', $subCate->desc ?? $subCate->description) }}</textarea>
                            @error('desc')
                                <div class="text-danger small mt-1" style="font-size: 0.7rem;">{{ $message }}</div>
                            @enderror
                        </div>
